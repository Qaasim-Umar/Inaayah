  ...
  return (
    <>
      ...
      <header ref={header} className="sticky top-0 z-50 border-b border-border/70 bg-surface shadow-sm" onBlur={(event) => { if (!event.currentTarget.contains(event.relatedTarget)) setIsOpen(false); }}>
        <Container>
          <nav aria-label="Primary navigation" className="flex min-h-20 items-center justify-between gap-3 lg:min-h-24">
            ...
            <div className="hidden items-center gap-6 xl:flex">
              {siteConfig.navigation.slice(1, 5).map((item) => <Link key={item.href} href={item.href} className="inline-flex min-h-12 items-center text-sm font-bold text-muted-foreground transition-colors hover:text-brand">{item.label}</Link>)}
            </div>
        ...
        {isOpen && (
          <div id="site-navigation-panel" className="menu-enter absolute inset-x-0 top-full max-h-[calc(100dvh-8rem)] overflow-y-auto rounded-b-3xl border-b border-border bg-surface shadow-soft xl:hidden">
            <Container className="pb-6 pt-2">
              <nav aria-label="Menu navigation" className="grid gap-1 sm:grid-cols-2 sm:gap-x-6">
                {siteConfig.navigation.map((item) => <Link key={item.href} href={item.href} onClick={() => setIsOpen(false)} aria-current={!item.href.includes('#') && pathname === item.href ? 'page' : undefined} className="group flex min-h-12 items-center justify-between rounded-xl px-3 py-3 text-sm font-bold text-brand transition-colors hover:bg-brand-soft"><span>{item.label}</span><ArrowUpRight aria-hidden="true" className="size-4 text-muted-foreground" /></Link>)}
              </nav>
